...
var TEMPLATE_DIR = import_path.default.resolve(__dirname, "templates");
var TEMPLATE_CONFIG_FILE = "template.json";

// src/commands/list.ts
var list = () => {
  for (const dir of import_fs.default.readdirSync(TEMPLATE_DIR)) {
    console.log(dir);
  }
};
...
var templates = () => {
  const arr = [];
  for (const dir of import_fs2.default.readdirSync(TEMPLATE_DIR)) {
    arr.push(dir);
  }
  return arr;
};
...
// src/commands/new.ts
var createTemplate = (template, options) => {
  let destination = options.name ?? process.cwd();
  if (!import_path2.default.isAbsolute(destination)) {
    destination = import_path2.default.resolve(process.cwd(), destination);
  }
  if (!templates().includes(template)) {
    throw new Error(`Template ${template} does not exist`);
  }
  const templatePath = import_path2.default.join(TEMPLATE_DIR, template);
  const templateConfig = JSON.parse(
    import_fs3.default.readFileSync(import_path2.default.join(templatePath, TEMPLATE_CONFIG_FILE)).toString()
  );
...
program.command("new <template>").description("Generate a template").option("-n, --name <name>", "The name of the file").action(createTemplate);
program.parse();
